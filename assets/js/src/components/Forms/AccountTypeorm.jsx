...
function AccountTypeForm({ onClose,  accountType, mode }) {
  const [name, setName] = useState('');
console.log(accountType);

  useEffect(() => {
    if (mode === 'edit' &&  accountType) {
  ...
  const handleSubmit = (e) => {
    ...
    if (mode === 'edit') {
      console.log('Updating  accountType:', { ... accountType, });
    } else {
      console.log('Creating  accountType:',);
    }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-semibold text-slate-900 dark:text-white">
                {mode === 'edit' ? 'Edit Account Type' : 'Create New Account Type'}
              </h2>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="flex justify-end gap-4 pt-4 mt-2 border-t border-border-light dark:border-border-dark">
              ...
              >
                {mode === 'edit' ? 'Save Changes' : 'Create Account Type'}
              </button>
            </div>
          </form>
        </div>
